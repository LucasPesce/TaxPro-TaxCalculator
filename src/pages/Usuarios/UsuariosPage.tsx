...
export const UsuariosPage: React.FC = () => {
    const { usuarios, handleCreateUsuario, handleUpdateUsuario, handleDeleteUsuario, handleRestoreUsuario } = useUsuariosManager();
                            ...
                            {usuarios.map(u => (
                                <tr key={u.id} style={{ borderBottom: '1px solid var(--border-color)', opacity: u.activo ? 1 : 0.6 }}>
                                    ...
                                    <td style={{ padding: '12px', display: 'flex', justifyContent: 'center', gap: '8px' }}>
                                        ...
                                        {u.activo ? (
                                            <Button variant="icon" onClick={() => handleDeleteUsuario(u.id)} title="Inhabilitar (Papelera)">
